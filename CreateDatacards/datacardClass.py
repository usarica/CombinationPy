...
# fai1 used in spin-parity and off-shell
	...
	#------------------------------------------------end bIncludingError
        ...
        if(self.bVBF):
            TemplateName += "_{0}".format(self.VBFcat)
        sigTemplatePdf_VBF = ROOT.RooHistPdf(TemplateName,TemplateName,ROOT.RooArgSet(CMS_zz4l_mass,D),sigTempDataHist)
	...
	####  ----------------------- end mekd -----------------------------------------------------------
        ...
        sigTemplateSDPdf_VBF = ROOT.RooHistPdf(TemplateSDName,TemplateSDName,ROOT.RooArgSet(SD),sigTempSDDataHist)
        ...
        if not self.bVBF:
            ...
            if (self.channel == self.ID_4mu):
                ...
                print "pol0 2p2f 4e: ",p0Zjet_2p2f.getVal()
                print "pol1 2p2f 4e: ",p1Zjet_2p2f.getVal()
                bkg_zjets_2p2f = ROOT.RooGenericPdf("bkg_zjetsTmp_2p2f","bkg_zjetsTmp_2p2f","(TMath::Landau(@0,@1,@2))*(1.+ TMath::Exp(@3+@4*@0))",ROOT.RooArgList(CMS_zz4l_mass,mlZjet_2p2f,slZjet_2p2f,p0Zjet_2p2f,p1Zjet_2p2f))
...
#            print "ggH rfv: ",rfvSigEff_ggH.getVal()
        ...
        w.importClassCode(ROOT.RooRelBWUFParam.Class(),True)
        ...
        if( FactorizedShapes ):
            if( self.channel == self.ID_4mu ):
                w.importClassCode(ROOT.RooFourMuMassShapePdf2.Class(),True)
                w.importClassCode(ROOT.RooFourMuMassRes.Class(),True)
            elif( self.channel == self.ID_4e ):
                w.importClassCode(ROOT.RooFourEMassShapePdf2.Class(),True)
                w.importClassCode(ROOT.RooFourEMassRes.Class(),True)
            elif( self.channel == self.ID_2e2mu ):
                w.importClassCode(ROOT.RooTwoETwoMuMassShapePdf2.Class(),True)
                w.importClassCode(ROOT.RooTwoETwoMuMassRes.Class(),True)
